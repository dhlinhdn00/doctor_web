...
const DataResult = ({ data }) => {
  ...
  return (
    <div className={style.data_container}>
      <h2 className={style.title}>{result ? result.date : dateNow}</h2>
        ...
        <div className={style.data_item}>
          ...
          <div className={`${style.data_circle_overlay} ${style.yellow_overlay}`}>
            <div className={`${style.data_circle}  ${style.yellow}`}>
              <strong>{result ? result.resp : 0}</strong>BPM
            </div>
          </div>
        </div>
        <div className={style.data_item}>
          ...
          >
            <div className={`${style.data_circle} ${style.red} ${style.center}`}>
              <strong>{result ? Math.round(result.heartrate) : 0}</strong>BPM
            </div>
          </div>
        </div>
        <div className={style.data_item}>
      ...
      >
        <p>Advice</p>
        {result ? result.advice : 'No advice'}
      </Modal> */}
    </div>
  )
}
